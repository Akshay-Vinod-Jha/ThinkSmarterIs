...
      }, 5000);
  ...
  return (
    ...
    <div className="w-full mt-4 grid grid-cols-1 gap-4 bg-[#1e1e1e] rounded-lg">
      ...
      {first && <div className="w-full flex flex-col px-1 md:px-8 mb-4 justify-start items-center gap-2">
        <textarea name="texttospeech" onChange={(e)=>{
setTextIs(e.target.value);
        }} ref={wordRef} id="texttospeech" cols="30" value={textIs} rows="10" className="w-full mx-4 py-2 px-3 text-white rounded-lg border-2 border-[#ffffff38] bg-black resize-none h-[20rem] font-lexend text-sm md:text-base" placeholder="Type Your Content Here.."></textarea>
        <OrangeButton onClick={clickHandler}>{request && <Loader/>}Convert To Speech</OrangeButton>
        </div>}
      {!first && <div className="w-full flex flex-col px-1 md:px-8 mb-4 justify-start items-center gap-2">
      <textarea name="texttospeech2" ref={wordRef2} onChange={(e)=>{wordRef2.current.value=transcript}} id="texttospeech2" cols="30" value={transcript} rows="10" className="w-full mx-4 py-2 px-3 text-white rounded-lg border-2 border-[#ffffff38] bg-black resize-none h-[20rem] font-lexend text-sm md:text-base" placeholder="Your Content Will Be Displayed Here.."></textarea>
       ...
       <button  
        ...
        setHold(true);}}
           onTouchEnd={()=>{SpeechRecognition.stopListening();setHold(false);}}
           onMouseUp={()=>{SpeechRecognition.stopListening();setHold(false);}}>{hold?"Leave To Stop Listening":"Hold To Start Listening"}{hold?<FaRegHandPaper/>:<LuGrab/>}</button>
           <button onClick={resetTranscript} className="w-full py-2 flex justify-center items-center gap-2 font-lexend text-sm md:text-base lg:text-lg text-white bg-[#fc0001] rounded-xl">Reset</button>
        </div><div className="w-full flex justify-end items-center">
            <Copy text={transcript}/>
            </div></div>}
    </div>
     {first && <div className="w-full flex flex-col my-4 font-lexend text-sm md:text-base lg:text-lg xl:text-xl justify-center items-center gap-2 bg-[#1E1E1E] rounded-xl">
